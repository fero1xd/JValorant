...
/**
 * Utility class for parsing api response
...
public class Utils {
    ...
    public static Match parseMatch(JSONObject data) {
        MatchMetadata metadata = getMetadata((JSONObject) data.get("metadata"));
        Players players = parseAllPlayers((JSONObject) data.get("players"));
        TeamsStatus teamsStatus = parseTeamsStatus((JSONObject) data.get("teams"));
        Round[] rounds = parseRounds((JSONArray) data.get("rounds"));
        Kill[] kills = parseKills((JSONArray) data.get("kills"));

        return new Match(metadata, players, teamsStatus, rounds, kills);
    }

    private static MatchMetadata getMetadata(JSONObject metadata) {
    ...
    private static Players parseAllPlayers(JSONObject players) {
    ...
    private static TeamsStatus parseTeamsStatus(JSONObject teams) {
    ...
    private static Round[] parseRounds(JSONArray roundsRaw) {
    ...
    private static Kill[] parseKills(JSONArray killsRaw) {
